...
Request::Request(const Service &svc, const std::vector<uint16_t> requestData)
    : m_service(svc), m_data(requestData) {}
...
std::ostream &operator<<(std::ostream &out, const Request &req) {
  out << "<Request: [" << req.m_service.getServiceName() << "] (subfunction "
      << +req.m_service.getSubfunction() << ") - " << 2 * (req.m_data.size())
      << " data bytes>";
  return out;
}

std::vector<uint8_t> Request::getPayload() { return m_service.serialize(); }
